...
#define ZE08_SENSOR_TIMEOUT_MS        60U
#define ZE08_RX_BYTE_TIMEOUT_MS       5U
#define ZE08_LEVEL_STEP_PPB           5000U

typedef struct
{
    uint16_t concentration_ppb;
    uint16_t full_range_ppb;
    uint16_t level_base_ppb;
    uint16_t concentration_hex;
...
static uint32_t g_ze08_last_query_tick;
...
static void FillHex5(uint8_t dest[5], uint16_t value)
{
    char buf[6];
    snprintf(buf, sizeof(buf), "%04XH", value);
    memcpy(dest, buf, 5);
}
...
static HAL_StatusTypeDef ZE08_ReadFrame(uint8_t *frame)
{
    ...
    while ((HAL_GetTick() - start_tick) < ZE08_SENSOR_TIMEOUT_MS) {
        if (HAL_UART_Receive(&huart1, &byte, 1, ZE08_RX_BYTE_TIMEOUT_MS) == HAL_OK) {
            if (idx == 0U) {
                if (byte != 0xFFU) {
                    continue;
                }
...
static uint8_t ZE08_ParseFrame(const uint8_t *frame, ZE08_Data_t *out)
{
    ...
    ppb = (uint16_t)(((uint16_t)frame[4] << 8) | frame[5]);
    out->concentration_ppb = ppb;
    out->full_range_ppb = (uint16_t)(((uint16_t)frame[6] << 8) | frame[7]);
    out->concentration_hex = ppb;
    out->level_base_ppb = (uint16_t)((ppb / ZE08_LEVEL_STEP_PPB) * ZE08_LEVEL_STEP_PPB);
...
static void ZE08_Init(void)
{
    ...
    g_ze08_last_query_tick = HAL_GetTick();
    memset(PC_SEND_data.AD0, '0', 5);
    memset(PC_SEND_data.AD1, '0', 5);
    memset(PC_SEND_data.AD2, '0', 5);
    memset(PC_SEND_data.AD3, '0', 5);
    memset(PC_SEND_data.AD4, '0', 5);
    memset(PC_SEND_data.AD5, '0', 5);
    memset(PC_SEND_data.AD6, '0', 5);
    memset(PC_SEND_data.AD7, '0', 5);
...
static void ZE08_PollAndUpdate(void)
{
    ...
    if ((ZE08_ReadFrame(g_ze08_rx_buf) == HAL_OK) && ZE08_ParseFrame(g_ze08_rx_buf, &g_ze08_data)) {
        Fill5Ascii(PC_SEND_data.AD0, g_ze08_data.concentration_ppb);   /* ppb */
        Fill5Ascii(PC_SEND_data.AD1, g_ze08_data.level_base_ppb);      /* 5000ppb 구간 시작값 */
        FillHex5(PC_SEND_data.AD2, g_ze08_data.concentration_hex);      /* ppb HEX */
        Fill5Ascii(PC_SEND_data.AD3, g_ze08_data.full_range_ppb);
    }
}
//#define TT
void pGC_main(void)
{
	...
	while(1){
		...
		if ((HAL_GetTick() - g_ze08_last_query_tick) >= ZE08_QUERY_PERIOD_MS) {
			ZE08_PollAndUpdate();
		}
		...
		switch(g_sys_tick_count){
			case 5 :
				if(send_1s_flag){
					send_1s_flag=0;
					/* USART1은 ZE08 센서 전용이므로 결과 프레임은 USART2로 송신 */
